...
describe("the file service", () => {
    it("should return an array of data (even dataset)", async () => {
        expect(await parseFile("tests/file/file.csv")).toEqual(expect.arrayContaining(evenDatasetResult))
    })

    it("should return an array of data (uneven dataset)", async () => {
        expect(await parseFile("tests/file/game-file.csv")).toEqual(expect.arrayContaining(unevenDatasetResult))
    })
})